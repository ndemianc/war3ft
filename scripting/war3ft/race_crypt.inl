...
public _CL_ULT_LocustEffect( parm[] )
{
	...
	if ( !is_user_alive( iVictim ) || !p_data_b[iVictim][PB_ISCONNECTED] )
	{
		...
		// sdemian
		client_print_color_msg( iAttacker, "%s %L", g_MODclient_color, iAttacker, "CLIENT_PRINT_CL_ULT_LOCUST_EFFECT_0");
		...
		// sdemian
		client_print_color_msg( iAttacker, "%s %L", g_MODclient_color, iAttacker, "CLIENT_PRINT_CL_ULT_LOCUST_EFFECT_2" );
	...
	else
	{
		...
		// sdemian
		client_print_color_msg( iAttacker, "%s %L", g_MODclient_color, iAttacker, "CLIENT_PRINT_CL_ULT_LOCUST_EFFECT_3", szName, iDamage );
	}
}
...
CL_SkillsOffensive( iAttacker, iVictim, iHitPlace )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( random_float( 0.0, 1.0 ) <= p_carrion[iSkillLevel-1] )
		{
			...
			Create_TE_SPRITETRAIL( vAttackerorigin, vVictimOrigin, g_iSprites[SPR_BEETLE], 35, 15, 2, 2, 6 );
