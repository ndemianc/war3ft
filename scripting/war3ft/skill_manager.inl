...
{
	// log_error( AMX_ERR_NATIVE, "id = %d, skill_id = %d, debug_id = %d", id, skill_id, debug_id );
...
SM_SetSkillLevel( id, skill_id, iLevel, iDebugID )
{
	...
	// Technically we shouldn't have a skill level EVER greater than MAX_SKILL_LEVEL right?
...
bool:SM_IsValidSkill( skill_id )
{
	...
	// log_error( AMX_ERR_NATIVE, "skill_id = %d", skill_id );
	return false;
}
...
SM_GetSkillOfType( id, type, iStart = 0 )
{
	...
	log_error( AMX_ERR_NATIVE, "[SM_GetSkillOfType line 439] id = %d, type = %d, iStart = %d ", id, type, iStart );
	return -1;
}
