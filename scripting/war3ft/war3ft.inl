...
public WC3_Precache()
{
	...
	// sdemian 	copy( g_szSprites[SPR_BEETLE]		, 63, "sprites/agrunt1.spr" );
	copy( g_szSprites[SPR_BEETLE]		, 63, "sprites/crypt_beetle.spr" );
	copy( g_szSprites[SPR_FIREBALL]		, 63, "sprites/zerogxplode.spr" );
	// copy( g_szSprites[SPR_BEAM]			, 63, "sprites/zbeam4.spr" );
	copy( g_szSprites[SPR_BEAM]			, 63, "sprites/night_elf_entangle.spr" );
  ...
  copy( g_szRaceSprites[RACE_NECROMANCER] , 63, "sprites/warcraft3/races/wc3_chameleon_01.spr"    );
	copy( g_szSprites[SPR_COLDSTRIKE]		, 63, "sprites/necromancer_coldstrike.spr" );
...
public WC3_Init()
{
	...
	// sdemian
	g_bAnkhDisabled = WC3_MapDisableCheck( "item_ankh.cfg" );
...
WC3_ShowBar( id )
{
	...
	if ( SHARED_IsOnTeam( id ) )
	{
		...
		if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
		{
			...
			if ( is_user_alive( id ) )
			{
				// Create_StatusText( id, 0, szString ); // old level item menu for alive player in the bottom
				set_hudmessage( 0, 255, 0, 0.02, 0.86, HUDMESSAGE_FX_FADEIN, 10.0, 540.0, 2.0, 3.0, HUD_XP );
				show_hudmessage( id, "%s", szString );
			}

			else
			{
				set_hudmessage( 160, 160, 160, 0.75, 0.90, HUDMESSAGE_FX_FADEIN, 10.0, 0.0, 2.0, 3.0, HUD_XP );
...
WC3_ShowSpecInfo( id, iTargetID )
{
	...
	if ( g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] > ITEM_NONE && g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] != ITEM_MOLE )
	{
		...
		if ( HasMoreThanOneRing && g_iShopMenuItems[iTargetID][ITEM_SLOT_ONE] == ITEM_RING )
			iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, "^n%s x%d", szItemName, g_iTotalRings[iTargetID] );
		else
			iMsgPos += formatex( szMsg[iMsgPos], 512-iMsgPos, "^n%s", szItemName );
	...
	if ( g_MOD == GAME_CSTRIKE || g_MOD == GAME_CZERO )
	{
		...
		if ( SM_GetSkillLevel( iVictim, ULTIMATE_VENGEANCE ) > 0 && iKiller != iVictim && !g_EndRound )
		{
			...
			if ( ULT_CanUserBlockUlt( iKiller ) )
			{
				...
				// sdemian
				client_print_color_msg( iVictim, "%s %L", g_MODclient_color, iVictim, "CLIENT_PRINT_WC3_DEATH" );
			}
