...
#define VENGEANCE_HEALTH		67			// Health the user should have after using his ult
...
WA_ULT_Vengeance( id )
{
	...
	if ( p_data_b[id][PB_ISPLANTING] )
	{
		...
		// sdemian
		client_print( id, print_center, "¬ы не можете использовать супер способность во врем€ установки бомбы!" );
		return;
	}
...
WA_Blink( id )
{
	...
	if ( iSkillLevel > 0 )
	{
		...
		if ( random_float( 0.0, 1.0 ) <= p_blink[iSkillLevel-1] )
		{
			...
			// client_print( id, print_chat, "%s %L", g_MODclient, id, "WARDEN_IMMUNE_TO_ULTIMATES" );
			// sdemian
			client_print_color_msg( id, "%s %L", g_MODclient_color, id, "WARDEN_IMMUNE_TO_ULTIMATES");
		}
	}
