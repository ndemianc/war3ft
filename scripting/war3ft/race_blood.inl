...
BM_PhoenixSpawn( id )
{
	...
	if ( iPhoenixID != -1 )
	{
		...
		if ( iPhoenixID != id )
		{
			...
			// sdemian
			client_print_color_msg( id, "%s %L", g_MODclient_color, id, "HAS_REVIVED_YOU", szCastername );
			// client_print( iPhoenixID, print_chat, "%s %L", g_MODclient, iPhoenixID, "YOU_HAVE_REVIVED", szSpawnerName );
			// sdemian
			client_print_color_msg( iPhoenixID, "%s %L", g_MODclient_color, iPhoenixID, "YOU_HAVE_REVIVED", szSpawnerName );
		}
	}
