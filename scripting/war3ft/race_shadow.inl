...
public _SH_DrawSerpentWard( parm[5] )
{
	...
	if ( bWardBombCheck )
	{
		if ( !SH_CheckWard( parm ) )
		{
			...
			// sdemian
			client_print_color_msg( id, "%s %L", g_MODclient_color, id, "CLIENT_PRINT_SH_DRAW_SERPENT_WARD" );
...
public _SH_ResetBigBadAttacker( id )
{
	...
	// sdemian
	client_print_color_msg( id, "%s %L", g_MODclient_color, id, "CLIENT_PRINT_SH_RESET_BIG_BAD_ATTACKER" );
}
